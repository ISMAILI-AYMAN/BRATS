...
import os
import numpy as np
from custom_datagen import imageLoader, imageLoader_with_rpn
...
# Ensure input images and masks are float32
...
metrics = ['accuracy', 
           sm.metrics.IOUScore(threshold=0.5)]  # Ensure dtype is float32
...
'''from MASKRCNN import mask_rcnn_model

model1 = mask_rcnn_model(IMG_HEIGHT=128, 
                        IMG_WIDTH=128, 
                        IMG_DEPTH=128, 
                        IMG_CHANNELS=3, 
                        num_classes=4)
...
model1.compile(optimizer='adam', 
              loss={'rpn_class': 'categorical_crossentropy', 
                    'rpn_bbox': 'mse', 
...
if gpus:
    try:
        ...
        print(e)
train_img_datagen1 = imageLoader_with_rpn(train_img_dir, train_img_list, train_mask_dir, train_mask_list, batch_size)
val_img_datagen1= imageLoader_with_rpn(val_img_dir, val_img_list, val_mask_dir, val_mask_list, batch_size)
...
model2 = nnunet_model(IMG_HEIGHT=128, 
                     IMG_WIDTH=128, 
                     IMG_DEPTH=128, 
                     IMG_CHANNELS=3, 
                     num_classes=4)
...
if gpus:
    try:
        ...
        print(e)
train_img_datagen2 = imageLoader(train_img_dir, train_img_list, train_mask_dir, train_mask_list, batch_size)
val_img_datagen2= imageLoader(val_img_dir, val_img_list, val_mask_dir, val_mask_list, batch_size)

# Proceed with your model training
with tf.device('/GPU:0'):
    history = model2.fit(
        train_img_datagen2,
        validation_data=val_img_datagen2,
        epochs=5,
        steps_per_epoch=steps_per_epoch,
        validation_steps=val_steps_per_epoch
    )

model2.save('brats_nnUnet.hdf5')
loss = history.history['loss']
val_loss = history.history['val_loss']
epochs = range(1, len(loss) + 1)
plt.plot(epochs, loss, 'y', label='Training loss')
