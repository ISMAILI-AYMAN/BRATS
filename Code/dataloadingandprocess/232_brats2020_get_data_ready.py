...
########################################
'''
# Process validation data without segmentation masks

validation_t2_list = sorted(glob.glob('BraTS2020_ValidationData/MICCAI_BraTS2020_ValidationData/*/*t2.nii'))
validation_t1ce_list = sorted(glob.glob('BraTS2020_ValidationData/MICCAI_BraTS2020_ValidationData/*/*t1ce.nii'))
validation_flair_list = sorted(glob.glob('BraTS2020_ValidationData/MICCAI_BraTS2020_ValidationData/*/*flair.nii'))

for img in range(len(validation_t2_list)):  # Iterate through validation images
    print("Now preparing validation image number: ", img)
    
    temp_image_t2 = nib.load(validation_t2_list[img]).get_fdata()
    temp_image_t2 = scaler.fit_transform(temp_image_t2.reshape(-1, temp_image_t2.shape[-1])).reshape(temp_image_t2.shape)
    
    temp_image_t1ce = nib.load(validation_t1ce_list[img]).get_fdata()
    temp_image_t1ce = scaler.fit_transform(temp_image_t1ce.reshape(-1, temp_image_t1ce.shape[-1])).reshape(temp_image_t1ce.shape)
    
    temp_image_flair = nib.load(validation_flair_list[img]).get_fdata()
    temp_image_flair = scaler.fit_transform(temp_image_flair.reshape(-1, temp_image_flair.shape[-1])).reshape(temp_image_flair.shape)
    
    # Combine the modalities into a single multichannel image
    temp_combined_images = np.stack([temp_image_flair, temp_image_t1ce, temp_image_t2], axis=3)
    ...
    np.save('BraTS2020_ValidationData/input_data_3channels/images/image_' + str(img) + '.npy', temp_combined_images)
    print(f"Validation image {img} saved.")